...
def parse_game(response):
    desc_array = response.xpath('//div[@class="src__Description-sc-1c7a3hi-2 dChPoq"]/text()').getall()
    preco = response.xpath('//div[@class="src__BestPrice-sc-17hp6jc-5 iyyimP priceSales"]/text()').getall()
    capa = response.xpath('//div[@class="image__WrapperImages-zie4e-1 iBvQLB"]/div/picture/img/@src').get()

    avaliacoes = response.xpath('//div[@class="review__Flex-sc-4m3nj8-0 review__Wrapper-sc-4m3nj8-1 cBgQU"]')
    ...
    trasportadora = response.xpath('//div[@class="offers-box__Wrapper-o10e3b-0 bRuUTP"]/p/strong/text()').get()
    vendedora = response.xpath('//div[@class="offers-box__Wrapper-o10e3b-0 bRuUTP"]/p/a/text()').get()
...
class GamesSpiderShoptime(scrapy.Spider):
    ...
    start_urls = ['https://www.shoptime.com.br/categoria/games/playstation-4/jogos-ps4']

    def parse(self, response):  # Pega todos os links da lista de jogos

        grid_games = response.xpath('//div[@class="row product-grid no-gutters main-grid"]')
        links_games = grid_games.xpath('//a[@class="Link-bwhjk3-2 iDkmyz TouchableA-p6nnfn-0 dVqjAx"]/@href').getall()
